...
Für die Variante mit reduction nähert sich der Speed up mit steigendem N der Anzahl der Kerne an.\\
Die manuelle Variante hat eine schlechtere Performance, als die Variante ohne parralelisierung. Dies 
legt wahrscheinlich an dem Aufwand der getrieben werden muss, um die Threads zu organisieren, daher anzuhalten und wieder zu starten bei jedem critical.
...
\begin{enumerate}
	\item {Memory read at$ ''demo_with_bugs.c'':27$ conflicts with a prior memory write at $''demo_with_bugs.c'':26$ (flow dependence)\\
		Kann behoben werden, indem die Schleife aufgeteilt wird in zwei Schleifen mit \#pragma omp prallel for simd.\\
		}
	\item {Memory read at $''demo_with_bugs.c'':50$ conflicts with a prior memory write at $''demo_with_bugs.c'':45$ (flow dependence)\\
		Kann behoben werden, indem das nowait statement weggelassen wird.
		} 
	\item {Memory read at $''demo_with_bugs.c'':66$ conflicts with a prior memory write at $''demo_with_bugs.c'':66$ (flow dependence)\\
	...
	\item {Memory read at $''demo_with_bugs.c'':98$ conflicts with a prior memory write at $''demo_with_bugs.c'':98$ (flow dependence)\\
