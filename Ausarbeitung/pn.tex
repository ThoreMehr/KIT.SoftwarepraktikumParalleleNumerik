...
\begin{table}
	...
	\label{pi_runtime}
\end{table}
Wie an der Tabelle \ref{pi_runtime} abgelesen werden kann, nähert sich der Speed up mit steigendem N der Speedup in der Variane mit reduction der Anzahl der Kerne an.\\
Die manuelle Variante hat eine schlechtere Performance, als die Variante ohne parralelisierung. Dies 
liegt wahrscheinlich an dem Aufwand der getrieben werden muss, um die Threads zu organisieren, daher anzuhalten und wieder zu starten bei jedem critical.
...
\begin{table}
	...
	\begin{tabular}{r||r||r|r||r|r||r|r}
		...
		\hline
	\end{tabular}
	\caption{Laufzeiten des Mandelbort Programms unserer Parallelisierung gegenüber der sequentiellen Version. Getestet wurde auf dem Rechner \emph{i82sn07}. Um Ungenauigkeiten auszugleichen wurden die Zeiten jeweils über $10$ (für große Eingaben über $5$) Durchläufe gemittelt.}
	\label{mandel_runtime}
\end{table}
Wie an der Tabbele \ref{mandel_runtime} abgelesen werden kann, beträgt die Beschleunigung für 2 Threads etwa 1,4, für 16 Threads etwa 4 und für 32 Threads etwa 8. Das die Beschleunigung schlächter ist, als bei der Berechnung von PI liegt meiner Meinung nach daran, das Speicherverwaltung und Caches einen wesentlichen Anteil der Laufzeit ausmachen.
...
\begin{enumerate}
	\item {Memory read at \textbf{''demo\_with\_bugs.c'':27} conflicts with a prior memory write at \textbf{''demo\_with\_bugs.c'':26} (flow dependence)\\
		Kann behoben werden, indem die Schleife aufgeteilt wird in zwei Schleifen mit \#pragma omp prallel for simd.
		}
	...
	\item {Memory read at \textbf{''demo\_with\_bugs.c'':66} conflicts with a prior memory write at \textbf{''demo\_with\_bugs.c'':66} (flow dependence)\\
		...
		Kann behoben werden, indem das x private gesetzt wird, indem das pragma zu \#pragma omp parallel for private(x) geändert wird.
		}
	\item{ Vom Thread Checker nicht erkannt.\\ Kann behoben werden in dem das Pragma auf \#pragma omp parallel for lastprivate(x) geändert wird.}
...
Um die Ausnutzung des Caches zu gewährleisten, wird die eine Matrix als Zeilen-, die andere als Splatenmatrix verarbeitet und nach dem ikj-Algorithmus multipliziert. Es wurden eine lineare Version, sowie drei parralelierte Versionen untersucht. Die Paralelisierten sind erstens eine Version, in der die innerste Schleife mit reduction parralelesiert wurde, zweitens eine in der die mittlere und eine in der die äußere schleife parralelesiert wurden. In den Tabellen \ref{2b_1000} und \ref{2b_2000} wird der Einfluss von Optimierungsoptionen vom GCC und ICC untersucht. Die Messungen wurden auf i82pc31 ausgeführt, da dieser über einen Intel CPU verfügt. Wie aus den Tabellen leicht abgelesen werden kann, sind die unterschiede bei den Stuffen O0 bis O3 gering. GCC schein etwas schnelleren Linearen Code zu generieren, ICC schnelleren parallelen. Wenn hingegen Vektorisierung benutzt werden kann, beim ICC mit der Option fast, wird der Code viel schneller, allerdings auf kosten der Größe, das Binary hat ~1,4 MB statt ~30kB. Im algemeinen ist das Binary, das vom ICC erzeugt wird etwa doppelt so groß, wie das, dass vom GCC erzeugt wird.
\begin{table}
	\centering
	\begin{tabular}{|r||r|r|r|r|r||r|r|r|r|}
		\hline
		 ...
		 Optionen &   O0 &   O1 &   O2 &    O3 & fast &   O0 &   O1 &   O2 &    O3 \\ \hline
		   linear & 9,26 & 1,19 & 1,17 &  1,17 & 0,25 & 9,56 & 1,16 & 1,16 &  1,16 \\ \hline
...
\begin{table}
	...
	\begin{tabular}{|r||r|r|r|r|r||r|r|r|r|}
		...
		\hline
		linear &74,58&9,9&9,88&9,86&2,27&76,75&9,76&9,75&9,73\\
		\hline
...
Anhand der Ausgabe von ompP kann leicht gesehen werden, dass wenige der Threads einen großteil der Arbeit leisten, was bei der Verwendung von reduction zu erwarten ist. Dies kann durch die explizite Angabe von Scheduling und Chuncksize verbessert werden.
\begin{table}
\begin{tabular}{|c|c|c|c|c|}
	\hline 
	& Chunks.=1 & Chunks.=10 & Chunks.=100 &Chunks.=1000   \\ 
	\hline 
	N=3000 &  &  &  &  \\  \hline
	static & 0,25 & 0,21 & 0,61 & 3,59 \\ 
	\hline 
	dynamic & 0,21 & 0,20 & 0,60 & 3,59 \\ 
	\hline 
	guided & 0,35 & 0,34 & 0,61 & 3,58 \\ 
	\hline 
	auto & 0,22 & 0,21 & 0,60 & 3,59 \\ 
	\hline 
	N=5000 &  &  &  &  \\ 
	\hline 
	static & 0,48 & 0,49 & 1,08 & 7,71 \\ 
	\hline 
	dynamic & 0,44 & 0,41 & 1,07 & 7,69 \\ 
	\hline 
	quided & 0,91 & 0,98 & 1,07 & 7,71 \\ 
	\hline 
	auto & 0,48 & 0,46 & 1,07 & 7,71 \\ 
	\hline 
\end{tabular} 
\caption{Einfluss von scheduleing und Chunksize auf die Laufzeit von Mandelbrot. Getestet wurde auf dem Rechner \emph{i82sn07}. Um Ungenauigkeiten auszugleichen wurden die Zeiten jeweils über $10$ Durchläufe gemittelt.}
\label{2c_mandelbrot}
\end{table}
