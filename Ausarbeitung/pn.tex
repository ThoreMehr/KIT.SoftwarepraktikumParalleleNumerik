...
E-Mail: fabian.miltenberer@student.kit.edu}
\and
...
% in the abstract
\begin{abstract}
Im Rahmen dieses Praktikums haben wir viel gelernt.
\end{abstract}
...
Da wir bei der Berechnung die zweidimensionale Struktur nicht explizit benötigen, lässt sich so der Verschnitt bei der Aufteilung auf die \emph{Warps} der GPU verkleinern.
...
\TODO{Tabelle mit Laufzeiten}

\subsubsection{Asynchrone Parallelisierungmethoden\\}
Links: \\
\url{http://scicomp.stackexchange.com/questions/16448/efficient-successive-over-relaxation-using-cuda} \\
\url{http://www.netlib.org/lapack/lawnspdf/lawn258.pdf} \\
\url{http://docs.nvidia.com/cuda/kepler-tuning-guide/#axzz4WWJfCg4J}

Bei \emph{relaxierten Methoden} nimmt man Rechenungenauigkeiten für bessere Performanz in Kauf.

Die Verwendung des Compilerflags \texttt{-use-fast-math} -- welches im Kontext von Relaxierung ebenfalls zu Gunsten der Berechnungszeit weniger Rechengenauigkeit garantiert -- brachte in der Laufzeit keinen Unterschied. Wir führen das darauf zurück, dass wir keine aufwändigeren Mathematikoperationen wie Kosinus oder Sinus berechnen lassen.

Da die GPU deutlich weniger Recheneinheiten für \emph{double precision} Fließkommazahlen hat als für \emph{single precision}, haben wir uns dazu entschieden, für die GPU Variante den Datentyp \emph{float} anstatt -- wie bei der CPU -- \emph{double} zu verwenden.
Die Ergebnisse sind unserer Auffassung nach dennoch vergleichbar, da  Tests mit \emph{float} anstatt \emph{double} auf der CPU keinen nennenswerten Geschwindigkeitsvorteil ergaben.

Der Blogeintrag \cite{CUDAMixedPrecision} geht noch weiter und zeigt auf, wie sich mittels 16-Bit Fließkommazahlen (\texttt{half}) die Geschwindigkeit weiter steigern lässt. Es wird eine doppelt so große Geschwindigkeit gegenüber 32-Bit Fließkommazahlen angegeben. Für unsere Anwendung ist die damit verbundene Ungenauigkeit möglicherweise zu groß. Ferner wird \texttt{half} erst ab CUDA 7.5 unterstützt, wohingegen wir mit CUDA 5.5 arbeiten.

\subsubsection{Eignung für andere Architekturen\\}
Zu Testzwecken haben wir die Implementierung aus Aufgabe \ref{p1a4c} für den Datentyp \texttt{float} umgeschrieben. Die damit gemessenen Laufzeiten unterscheiden sich unwesentlich von denen bei Verwendung mit \texttt{double}. Für die CPU scheint diese Art der Relaxierung daher wenig gewinnbringend. Wir führen das darauf zurück, dass CPUs heutzutage darauf optimiert sind, mit \texttt{double} genau so schnell zu rechnen wie \texttt{float}.
...
\subsubsection{}
\TODO{Eignung aus Mathematischer Sicht}
...
\subsubsection{}
Bei der Implementierung von \textbf{Algorithmus 2} ist uns aufgefallen, dass Einträge, die in $A$ 0 sind, auch in $L$ auf 0 gesetzt werden. Ferner gilt $U=L^T$ und $A$ ist symmetrisch. Daraus ergibt sich für die Einträge von $A$, $L$ und $U$: $a_{i,j}=0 \Rightarrow l_{i,j}=l_{j,i}=u_{i,j}=u_{j,i}=0$. Somit eignet sich die Speichermethodik aus der vorherigen Teilaufgabe auch für die Matrizen $L$ und $U$ aus Algorithmus 2. Beim Implementieren ist uns aufgefallen, dass $L$ und $U$ nicht mit 0 initialisiert sein sollten, da ansonsten Werte $NaN$ in den Ergebnismatrizen auftauchen. Mit der Einheitsmatrix als Startwerte von $L$ und $U$ leistet der Algorithmus das Gewünschte.

Für \textbf{Algorithmus 1} wählen wir nun als Indexmenge $S_U$ genau die Einträge, die nach Algorithmus 2 in $U$ ungleich $0$ sein können. Damit ist auch die Validität unserer gewählten Speichermethodik für diesen Algorithmus garantiert, da auch hier $L=U^T$ gilt und $\forall (i,j)\notin S_U:u_{i,j}=l_{j,i}=0$.
...
\subsubsection{}
%Im ersten Projekt -- genauer Aufgabe \ref{p1a5} -- haben wir festgestellt, dass mit zunehmendem $l$ der lokale Fehler geschrumpft ist. Wir haben daher für diese Aufgabe $l=9$ verwendet, größere $l$ waren uns aus Speicher-technischen Gründen nicht möglich ($l=9\Rightarrow d=511\Rightarrow n=261.121\Rightarrow$ Speicherverbrauch von mehr als $\frac{1}{2}n^2\cdot 8$ Bytes $>>2$GB).

%Um den maximal zulässigen lokalen Fehler von $\epsilon_i\leq10^-{-5}$ einzuhalten, mussten wir auf unsere mit dem Datentyp \texttt{double} implementierte Variante aus Aufgabe \ref{p2a3} zurückgreifen. Mit \texttt{float}-Genauigkeit war es uns unabhängig vom gewählten $\varepsilon_{Error}$ nicht möglich, die Bedingung einzuhalten.
%Für \texttt{double} konnten wir mit $\varepsilon_{Error}\leq10^{-32}$ die Bedingung mit $\epsilon_i\leq1,257\times10^{-5}$ erfüllen, wobei kleinere Werte hier keinerlei Verbesserung der Genauigkeit des Ergebnisses mehr ergaben. Wir führen dies auf sowohl auf Rechenungenauigkeiten, als auch die gewählte Gitterauflösung zurück.



\TODO{Vorkonditionierter Löser}
...
\begin{equation*}
M(\mathtt{rawdata1})=M(\mathtt{rawdata2})=max_x\cdot max_y\cdot max_z\cdot 19\cdot8
\end{equation*}
Bytes benötigt.
...
\end{split}
\end{equation*}
Bytes, wobei $M$ nicht größer als der verfügbare globale Speicher sein darf, also $M\leq 2.092.957.696$.
Bei Würfeln sind alle Kanten gleich lang, somit gilt $max_x=max_y=max_z$ und der Speicherbedarf vereinfacht sich zu
\begin{equation*}
