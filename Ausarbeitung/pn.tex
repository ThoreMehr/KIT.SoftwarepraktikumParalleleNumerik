...
\end{lstlisting}
Wie in der Auflistung \ref{helloworldoutput} zu sehen, folgt die Reihenfolge der ausgeführten Fäden keinem bestimmten Muster. Die Reihenfolgen zwischen verschiedenen Ausführungen sind in der Regel verschieden.
...
\subsubsection{}\label{p1a4a}
	...
	\end{equation}
	Nach der Definition von $A$ ist klar, dass $a_{j,j}=4$ gilt. Für $i\neq j$ gilt $a_{i,j}\in\{0,-1\}$. Um die dünne Struktur von $A$ weiter auszunutzen, betrachten wir anschaulich, von welchen Gitterpunkten die Berechnung für einen Gitterpunkt $(x,y)$ konkret abhängt (genau das wir von $A$ beschrieben). Dargestellt wird dies in Abbildung\ref{gsv_depend}, wobei $A$ genau für die vier Nachbarknoten den Wert $-1$ annimmt. Durch diese Betrachtung zerfällt die Summe der Zuweisung \ref{gsv_unifiedu} zu vier bedingten Additionen, wie in Code \ref{conditionalsum} illustriert.
...
if (j % d < d - 1) sum += u[j + 1]; // Righter Nachbarknoten
...
In Aufgabe \ref{p1a4b} haben wir erläutert, dass eine naive Herangehensweise an das Parallelisierungsproblem nicht zielführend ist. Insbesondere haben wir hierfür die zahlreichen Abhängigkeiten innerhalb der Berechnung einer Iterierten verantwortlich gemacht. Um also das Gauß-Seidel-Verfahren nun doch effizient zu parallelisieren, ist eine übergeordnete Betrachtung des Problem von Nöten.
		...
		\begin{figure}
			\begin{center}
				...
				\begin{tabular}{|c|c|c|c|c|c|c|}
					\hline
					\cellcolor{cbblack}$u^{k+1}_{0,0}$ & \cellcolor{cbwhite}$u^{k+1}_{1,0}$ & \cellcolor{cbblack}$u^{k}_{2,0}$ & \cellcolor{cbwhite}$u^{k}_{3,0}$ & \cellcolor{cbblack}$u^{k-1}_{4,0}$ & \cellcolor{cbwhite}$u^{k-1}_{5,0}$ & \cellcolor{cbblack}$u^{k-2}_{6,0}$ \\
					\hline
					\cellcolor{cbwhite}$u^{k+1}_{0,1}$ & \cellcolor{cbblack}$u^{k}_{1,1}$ & \cellcolor{cbwhite}$u^{k}_{2,1}$ & \cellcolor{cbblack}$u^{k-1}_{3,1}$ & \cellcolor{cbwhite}$u^{k-1}_{4,1}$ & \cellcolor{cbblack}$u^{k-2}_{5,1}$ & \cellcolor{cbwhite}$u^{k-2}_{6,1}$ \\
					\hline
					\cellcolor{cbblack}$u^{k}_{0,2}$ & \cellcolor{cbwhite}$u^{k}_{1,2}$ & \cellcolor{cbblack}$u^{k-1}_{2,2}$ & \cellcolor{cbwhite}$u^{k-1}_{3,2}$ & \cellcolor{cbblack}$u^{k-2}_{4,2}$ & \cellcolor{cbwhite}$u^{k-2}_{5,2}$ & \cellcolor{cbblack}$u^{k-3}_{6,2}$ \\
					\hline
					\cellcolor{cbwhite}$u^{k}_{0,3}$ & \cellcolor{cbblack}$u^{k-1}_{1,3}$ & \cellcolor{cbwhite}$u^{k-1}_{3,3}$ & \cellcolor{cbblack}$u^{k-2}_{3,3}$ & \cellcolor{cbwhite}$u^{k-2}_{5,3}$ & \cellcolor{cbblack}$u^{k-3}_{5,3}$ & \cellcolor{cbwhite}$u^{k-3}_{7,3}$ \\
					\hline
					\cellcolor{cbblack}$u^{k-1}_{0,4}$ & \cellcolor{cbwhite}$u^{k-1}_{1,4}$ & \cellcolor{cbblack}$u^{k-2}_{2,4}$ & \cellcolor{cbwhite}$u^{k-2}_{3,4}$ & \cellcolor{cbblack}$u^{k-3}_{4,4}$ & \cellcolor{cbwhite}$u^{k-3}_{5,4}$ & \cellcolor{cbblack}$u^{k-4}_{6,4}$ \\
					\hline
					\cellcolor{cbwhite}$u^{k-1}_{0,5}$ & \cellcolor{cbblack}$u^{k-2}_{1,5}$ & \cellcolor{cbwhite}$u^{k-2}_{2,5}$ & \cellcolor{cbblack}$u^{k-3}_{3,5}$ & \cellcolor{cbwhite}$u^{k-3}_{4,5}$ & \cellcolor{cbblack}$u^{k-4}_{5,5}$ & \cellcolor{cbwhite}$u^{k-4}_{6,5}$ \\
					\hline
					\cellcolor{cbblack}$u^{k-2}_{0,6}$ & \cellcolor{cbwhite}$u^{k-2}_{1,6}$ & \cellcolor{cbblack}$u^{k-3}_{2,6}$ & \cellcolor{cbwhite}$u^{k-3}_{3,6}$ & \cellcolor{cbblack}$u^{k-4}_{4,6}$ & \cellcolor{cbwhite}$u^{k-4}_{5,6}$ & \cellcolor{cbblack}$u^{k-5}_{6,6}$ \\
					\hline
				\end{tabular}
			\end{center}
	...
	\begin{table}
		...
		\end{tabular}
	\end{center}
	\caption{Laufzeiten unserer Parallelisierung gegenüber der sequentiellen Version.}
	\label{gsv_runtime}
	\end{table}

	Die Laufzeiten sind Tabelle \ref{gsv_runtime} zu entnehmen.
...
Da in Gleichung \ref{fgleichung} $M,N \in\mathbb{N}$ beliebig, wählen wir der Einfachheit halber $M=N=1$ zur Lösung dieser Teilaufgabe. Damit ergibt sich
	...
	\begin{table}
		\begin{tabular}{r||r|r|r|r|r|r|r|r}
			...
			\hline
			Maximaler Fehler & 0,234 & 0,053 & 0,0130 & 0,00326 & 0,000866 & 0,000326 & 0,00174 \\
			Iterationen & 21 & 64 & 175 & 409 & 1148 & 3515 & 11043
		\end{tabular}
